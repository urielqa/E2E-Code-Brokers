...
window.addEventListener('DOMContentLoaded', function() {
  // Garante que só a tela de login aparece ao iniciar
  ...
  if (redirectSection) redirectSection.classList.add('hidden');
  var loaderSplash = document.getElementById('loaderSplash');
  if (loaderSplash) loaderSplash.classList.add('hidden');
...
function login() {
  ...
  if (!validarSenhaForte(senha)) {
    msg.textContent = MSG_SENHA_FORTE + " Por favor, digite sua senha corretamente.";
    return;
  }
  let user;
  if (isCpf) {
    // Aceita CPF digitado com pontos/traço ou só números
  ...
  if (user) {
    ...
    setTimeout(() => {
      document.getElementById('redirectSection').classList.add('hidden');
      mostrarPortal();
    }, 2000);
  } else {
...
function cadastrarUsuario() {
  ...
  // Validação: nome deve conter apenas letras e pelo menos um espaço (nome e sobrenome)
  const nomeValido = /^[A-Za-zÀ-ÖØ-öø-ÿ]+( [A-Za-zÀ-ÖØ-öø-ÿ]+)+$/.test(nome);
  if (!nomeValido) {
    const msg = document.getElementById("cadastroMsg");
    msg.textContent = "Digite um nome completo válido (sem números, com nome e sobrenome).";
    return;
  }
  ...
  // Aceita CPF nos formatos 000.000.000-00 ou apenas números
  let cpf = cpfOriginal.replace(/[^\d]/g, '');
  const regexCpfPontuado = /^\d{3}\.\d{3}\.\d{3}-\d{2}$/;
  const regexCpfNumeros = /^\d{11}$/;
  if (!cpfOriginal || !(regexCpfPontuado.test(cpfOriginal) || regexCpfNumeros.test(cpfOriginal))) {
    msg.textContent = "Digite um CPF válido no formato 000.000.000-00 ou apenas números.";
    return;
  }
  ...
  if (!confirmar) {
    msg.textContent = "Por favor, preencha o campo de confirmação de senha.";
    return;
  }
  // Validação de nome
  if (nome.length < 6 || nome.length > 30) {
    msg.textContent = "O nome deve conter entre 6 e 30 caracteres.";
    return;
  }
  const regexPermitido = /^[A-Za-zÀ-ú\s]+$/;
  if (!regexPermitido.test(nome)) {
    msg.textContent = "O nome deve conter apenas letras e espaços, sem números ou caracteres especiais.";
    return;
  }
  ...
  // Removida a validação de nome único. Permite nomes iguais.
  // Validação de celular (10 ou 11 dígitos)
  const celularLimpo = celular.replace(/[^\d]/g, '');
  if (celularLimpo.length < 10 || celularLimpo.length > 11) {
    msg.textContent = "O celular deve conter DDD e ter entre 10 e 11 dígitos.";
    return;
  }
  // Validação de e-mail (aceita .com.br, .org, etc)
  const regexEmail = /^[\w-.]+@[\w-]+(\.[a-z]{2,})+$/i;
  ...
  // Cadastro válido
  // Corrigido: saldo inicial atualizado para 200000.00 e carteira vazia
  const novoUsuario = {
    nome,
...
if (typeof logout !== 'function') {
  function logout() {
    ...
    document.getElementById('transferSection').classList.add('hidden');
    document.getElementById('redirectSection').classList.add('hidden');
    document.getElementById('loaderSplash').classList.add('hidden');
    var tabs = document.getElementById('tabs');
    if (tabs) tabs.classList.add('hidden');
...
function mostrarLoaderSplash(callback) {
  const loader = document.getElementById("loaderSplash");
...
function voltarPortal() {
  ...
  var graficoSection = document.getElementById('graficoSection');
  if (graficoSection) graficoSection.classList.add('hidden');
  // Mostra loaderSplash por 1.5s antes de voltar ao dashboard
  var loaderSplash = document.getElementById('loaderSplash');
  if (loaderSplash) loaderSplash.classList.remove('hidden');
  setTimeout(() => {
    if (loaderSplash) loaderSplash.classList.add('hidden');
    document.getElementById('portal').classList.remove('hidden');
    // Esconde o loader caso esteja visível
    var loaderDiv = document.getElementById('loader');
    if (loaderDiv) loaderDiv.classList.add('hidden');
  }, 1500);
}
